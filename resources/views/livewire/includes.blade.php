<section>
    <h1 class="text-lg font-bold">Incluye</h1>
        ...
        <div class="card-body">
            @foreach ($includes as $include)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($include->description, 100) !!}</p>
                    <button wire:click="edit({{ $include->id }})" title="Editar {{ $include->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button wire:click="delete({{ $include->id }})" title="Eliminar {{ $include->name }}"
