<div>
    <div x-data="{ open: false }">
        <div class="flex justify-between">
            <h1 class="text-2xl font-bold mb-4">Lista de combo amigo</h1>
            <div>
        ...
        <div x-show="open" class="fixed inset-0 overflow-y-auto flex items-center justify-center z-50">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all w-full max-w-4xl mx-auto">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    <h1 class="pl-5 text-3xl font-semibold mb-4">Crear nuevo combo amigo</h1>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 px-5">
                        ...
                        <div>
                            <label for="price" class="block text-sm font-medium text-gray-700">Precio
                                cliente:</label>
                            <input type="text" wire:model="price" id="price" placeholder="1800"
                                class="mb-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                            @error('price')
                                <span class="text-red-400 text-xs">{{ $message }}</span>
                            @enderror
                        ...
                        <div>
                            <label for="price_real" class="block text-sm font-medium text-gray-700">Precio
                                proveedor:</label>
                            <input type="text" wire:model="price_real" id="price_real" placeholder="1500"
                                class="mb-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                            @error('price_real')
                                <span class="text-red-400 text-xs">{{ $message }}</span>
                            @enderror
            ...
            <div class="shadow overflow-hidden border-b border-gray-200 bg-white sm:rounded-lg">
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="bg-white">
                        @foreach ($friendCombos as $friendCombo)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $friendCombo->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $friendCombo->price }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $friendCombo->price_real }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $friendCombo->category->name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('editPhototrip', $friendCombo) }}"
                                        title="Editar {{ $friendCombo->name }}"
                                        class="text-green-600 hover:text-green-900 mr-2">
                                        <i class="fa-regular fa-image text-xl"></i>
                                    </a>
                                    <button wire:click="edit({{ $friendCombo->id }})" @click="open = true"
                                        title="Editar {{ $friendCombo->name }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                                        <i class="fas fa-edit text-xl"></i>
                                    </button>
                                    <button onclick="confirmDeletion({{ $friendCombo->id }})"
                                        title="Eliminar {{ $friendCombo->name }}" class="mr-3 text-red-600 hover:text-red-900">
                                        <i class="fas fa-trash-alt text-xl"></i>
                                    </button>
                                    <a href="{{ route('comboamigos.complement', $friendCombo) }}"
