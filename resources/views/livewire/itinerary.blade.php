<section>
    <h1 class="text-lg font-bold">Itinerario</h1>

    <article class="card">
        <div class="card-body bg-gray-100">
            <form wire:submit="save">
                <div class="mt-3 flex justify-center items-center">
                    <label for="title" class="text-xs font-bold mr-3">Titulo:</label>
                    <input wire:model="title" id="title"
                        class="text-xs block w-full p-2 mt-1 border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <div class="mt-3">
                    <label for="title" class="text-xs font-bold mr-3">Descripción:</label>
                    <textarea wire:model="description" id="description"
                        class="text-xs block w-full p-2 mt-1 border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"></textarea>
        ...
        <div class="card-body">
            @foreach ($itineraries as $itinerary)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <label class="text-xs">{{ $itinerary->title }}</label>
                    <p class="text-xs">{!! Str::limit($itinerary->description, 100) !!}</p>
                    <button wire:click="edit({{ $itinerary->id }})" title="Editar {{ $itinerary->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="itineraries({{ $itinerary->id }})" title="Eliminar {{ $itinerary->name }}"
