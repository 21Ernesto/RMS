<section>
    <h1 class="text-lg font-bold">Galería de carros</h1>

    <article class="card">
        <div class="card-body bg-gray-100">
            <form wire:submit="save">
                <div class="mt-3">
                    <label for="image" class="text-xs">Imagen:</label>
                    <input wire:model="image" id="image" type="file" class="py-2 rounded text-xs w-full">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-7 gap-2">
            @foreach ($images as $image)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <img src="{{ asset($image->image) }}" class="h-20 w-20 rounded" alt="">
                    <button wire:click="edit({{ $image->id }})" title="Editar {{ $image->image }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button wire:click="delete({{ $image->id }})" title="Eliminar {{ $image->image }}"
