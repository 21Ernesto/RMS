        ...
        <div class="card-body">
            @foreach ($messages as $message)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($message->description, 100) !!}</p>
                    <button wire:click="edit({{ $message->id }})" title="Editar {{ $message->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button wire:click="delete({{ $message->id }})" title="Eliminar {{ $message->name }}"
