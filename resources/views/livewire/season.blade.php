<div>
    <div x-data="{ open: false }">
        <div class="flex justify-between">
            <h1 class="text-2xl font-bold mb-4">Lista de temporadas</h1>
            <div>
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all w-full max-w-3xl mx-auto">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    <h1 class="pl-5 text-3xl font-semibold mb-4">
                        {{ $editId ? 'Actualizar temporada' : 'Crear nueva temporada' }}</h1>
                    ...
                    <div class="px-5 mb-3">
                        ...
                        <input type="datetime-local" wire:model="datestart" id="datestart"
                            class="border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                        @error('datestart')
                            <span class="text-red-400 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="px-5 mb-3">
                        ...
                        <input type="datetime-local" wire:model="dateend" id="dateend"
                            class="border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                        @error('dateend')
                            <span class="text-red-400 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="px-5 mb-3">
                        ...
                        <input type="number" step="0.1" wire:model="percentage" id="percentage" placeholder="10"
                            class="border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full">
                        @error('percentage')
                            <span class="text-red-400 text-xs">{{ $message }}</span>
                        @enderror
                    ...
                    <div wire:transition>
                        @foreach ($seasons as $season)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">{{ $season->datestart }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $season->dateend }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $season->percentage }} %</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-end">
                                    <button wire:click="edit({{ $season->id }})" @click="open = true"
                                        title="Editar {{ $season->name }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button wire:click="delete({{ $season->id }})" title="Eliminar {{ $season->name }}"
