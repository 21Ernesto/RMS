<section>
    <h1 class="text-lg font-bold">Horarios</h1>
        ...
        <div class="card-body">
            @foreach ($schedules as $schedules)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($schedules->description, 100) !!}</p>
                    <button wire:click="edit({{ $schedules->id }})" title="Editar {{ $schedules->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="schedules({{ $schedules->id }})" title="Eliminar {{ $schedules->name }}"
