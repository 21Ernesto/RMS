<div>
    <div x-data="{ open: false }">
        <div class="flex justify-between">
            <h1 class="text-2xl font-bold mb-4">Lista de paquetes</h1>
            <div>
        ...
        <div x-show="open" class="fixed inset-0 overflow-y-auto flex items-center justify-center z-50">
            ...
            <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all w-full max-w-4xl mx-auto">
                <form wire:submit.prevent="save" class="mb-4 mt-4">
                    <h1 class="pl-5 text-3xl font-semibold mb-4">Crear nuevo paquetes</h1>
                        ...
                        @foreach ($packages as $package)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('editPhototrip', $package) }}"
                                        title="Editar {{ $package->name }}"
                                        class="text-green-600 hover:text-green-900 mr-2">
                                        <i class="fa-regular fa-image text-xl"></i>
                                    </a>
                                    <button wire:click="edit({{ $package->id }})" @click="open = true"
                                        title="Editar {{ $package->name }}"
                                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                                        <i class="fas fa-edit text-xl"></i>
                                    </button>
                                    <button onclick="packages({{ $package->id }})"
                                        title="Eliminar {{ $package->name }}"
                                        class="mr-3 text-red-600 hover:text-red-900">
                                        <i class="fas fa-trash-alt text-xl"></i>
                                    </button>
                                    <a href="{{ route('packages.hotel', $package) }}" title="Crear hotel" class="mr-3 text-green-600 hover:text-green-900">
