        ...
        <div class="card-body">
            @foreach ($notes as $notes)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($notes->description, 100) !!}</p>
                    <button wire:click="edit({{ $notes->id }})" title="Editar {{ $notes->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button wire:click="delete({{ $notes->id }})" title="Eliminar {{ $notes->name }}"
