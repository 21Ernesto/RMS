<section>
    <h1 class="text-lg font-bold">Temporada baja</h1>
    ...
    <article class="card mt-4">
        <div class="card-body">
            @foreach ($lowSeasons as $lowSeasons)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($lowSeasons->description, 100) !!}</p>
                    <button wire:click="edit({{ $lowSeasons->id }})" title="Editar {{ $lowSeasons->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="lowSeasons({{ $lowSeasons->id }})" title="Eliminar {{ $lowSeasons->name }}"
