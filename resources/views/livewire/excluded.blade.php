<section>
    <h1 class="text-lg font-bold">No incluye</h1>
        ...
        <div class="card-body">
            @foreach ($excludeds as $excluded)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($excluded->description, 100) !!}</p>
                    <button wire:click="edit({{ $excluded->id }})" title="Editar {{ $excluded->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="excludeds({{ $excluded->id }})" title="Eliminar {{ $excluded->name }}"
