<section>
    <h1 class="text-lg font-bold">Alimentos</h1>
        ...
        <div class="card-body">
            @foreach ($foods as $food)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($food->description, 100) !!}</p>
                    <button wire:click="edit({{ $food->id }})" title="Editar {{ $food->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button wire:click="delete({{ $food->id }})" title="Eliminar {{ $food->name }}"
