<section>
    <h1 class="text-lg font-bold">Sugerencias</h1>
        ...
        <div class="card-body">
            @foreach ($suggestions as $suggestion)
                <div class="border-b border-gray-200 mb-2 pb-2">
                    <p class="text-xs">{!! Str::limit($suggestion->description, 100) !!}</p>
                    <button wire:click="edit({{ $suggestion->id }})" title="Editar {{ $suggestion->name }}"
                        class="text-indigo-600 hover:text-indigo-900 mr-2">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="suggestions({{ $suggestion->id }})" title="Eliminar {{ $suggestion->name }}"
