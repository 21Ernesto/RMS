...
class Promotion extends Component
{
    ...
    {
        $promotions = Trip::where('name', 'like', '%' . $this->search . '%')->where('type_trips', 'promotions')->paginate(8);
        $categories = Category::where('key', 'promotions')->get();

        return view('livewire.promotion', compact('promotions', 'categories'))->layout('layouts.admin');
    }
